...
#runs through other methods to get an array of data tables each containing pitches of each pitch type
def get_pitch_type_tables():
    bigdata = []
    f = 0
    pitchtypetable = []
    counter = 0
    while counter < 7:
        pitchtypetable = pitchtypedata(pitchtypes[counter])
        if pitchtypetable.size > 0:
            bigdata.append(pitchtypetable)
        counter = counter + 1
    # reindexing each pitch type table
    for j in bigdata:
        length = len(j)
        newindy = []
        for x in range(length):
            newindy.append(x)
        index = pd.Index(newindy)
        bigdata[f] = bigdata[f].set_index(index)
        f = f + 1
    return bigdata

# Process to Mannually Pull collumns and Create a data frame with
def data_from_player(name):
    n = 0
    k = 0
    l = 0
    rows_where_threw = []
    ...
    # makes an array with the row numbers of the pitches where the pitcher named throws
    with open(csv_file, 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            pitcher_name_check.append(row.get('Pitcher'))
    length_csv = len(pitcher_name_check)
    while n < length_csv:
        if pitcher_name_check[n] == name:
            rows_where_threw.append(n)
        n=n+1
    #print(rows_where_threw)

    with open(csv_file, 'r') as f:
        pitchcount = len(rows_where_threw)
        reader = csv.DictReader(f)
        for row in reader:
            if l == rows_where_threw[k]:
                pitcher_name.append(row.get('Pitcher'))
                pitch_types.append(row.get('TaggedPitchType'))
                if row.get('RelSpeed') == '':
                    velos.append(None)
                else:
                    velos.append(float(row.get('RelSpeed')))
                if row.get('VertApprAngle') == '':
                    vaa.append(None)
                else:
                    vaa.append(float(row.get('VertApprAngle')))                                   
                if row.get('SpinRate') == '':
                    spin_rates.append(None)
                else:
                    spin_rates.append(float(row.get('SpinRate')))
                if row.get('InducedVertBreak') == '':
                    vb.append(None)
                else:
                    vb.append(float(row.get('InducedVertBreak')))
                if row.get('HorzBreak') == '':
                    hb.append(None)
                else:
                    hb.append(float(row.get('HorzBreak')))
                if row.get('SpinAxis') == '':
                    tilt.append(None)
                else:
                    tilt.append(float(row.get('SpinAxis')))
                if row.get('RelHeight') == '':
                    relheight.append(None)
                else:
                    relheight.append(float(row.get('RelHeight')))
                if row.get('RelSide') == '':
                    relside.append(None)
                else:
                    relside.append(float(row.get('RelSide')))
                if row.get('Extension') == '':
                    extension.append(None)
                else:
                    extension.append(float(row.get('Extension')))
                if row.get('PitchCall') == '':
                    pitch_result.append(None)
                else:
                    pitch_result.append(row.get('PitchCall'))
                if k < (pitchcount - 1):
                    k = k + 1
            l = l + 1       
    #converts into data table        
    data = {
            'pitcher name' : pitcher_name,
            'pitch type': pitch_types,
            'velo' : velos,
            'spin rate' : spin_rates,
            'vertical break' : vb,
            'horizontal break' : hb,
            'spin axis' : tilt,
            'vaa' : vaa,
...
#creates a data table just with a specific pitch type
def pitchtypedata(pitchtype):
    #Takes Data From Table DF which includes all pitch types and just makes 
    # A data frame with a specific pitch type for analysis
    table = data_from_player(names[name_counter])
    pitchyo = table[(table['pitch type'] == pitchtype)]
    return pitchyo

#Averages Pitch Data from each Individual Pitch Type        
def average_pitch_types():
    i=0
    table = get_pitch_type_tables()
    limit = len(table)
    averages = []
    while i < limit:
        temp_avg = []
        dt = pd.DataFrame(table[i])
        #print(dt)
        #adds pitch type to list first
        temp_avg.append(dt.at[0,'pitch type'])
        #creates data frame of means
        df2 = dt.mean(numeric_only=True)
        #print(df2)
        #adds means rounded to two decimals to temporary average storage
        for x in range(len(df2)):
            if (x == 0 or x == 2 or x == 3):
                temp_avg.append("%.1f" % round(df2[x],1))
            else:
                temp_avg.append("%.2f" % round(df2[x],2))
        #adds max velo and spin rate for each pitch onto list
        temp_avg.insert(2,"%.1f" % round(dt['velo'].max(),1))
        temp_avg.insert(4,"%.2f" % round(dt['spin rate'].max(),0))
        if temp_avg[4] == 'nan':
            temp_avg[4] = 'N/A'
        else:
            temp_avg[4] = str(int(float(temp_avg[4])))
        if temp_avg[3] == 'nan':
            temp_avg[3] = 'N/A'
        else:
            temp_avg[3] = str(int(float(round(float(temp_avg[3]),0))))
        temp_avg.insert(1,str(len(dt)))
        #Adds Strike%
        strikes = 0
        swings = 0
        whiffs = 0
        for d in range(len(dt)):
            if dt.at[d,'call'] in ('FoulBall', 'StrikeCalled', 'StrikeSwinging', 'InPlay'):
                strikes = strikes + 1
            if dt.at[d,'call'] in ('FoulBall', 'StrikeSwinging', 'InPlay'):
                swings = swings + 1
            if dt.at[d,'call'] == 'StrikeSwinging':
                whiffs = whiffs + 1
        strike_percentage = round(100*(strikes/len(dt)),0)
        temp_avg.append(str(str(int(strike_percentage)) + '%'))
        if swings > 0:
            whiff_percentage = round(100*(whiffs/swings),0)
            temp_avg.append(str(str(int(whiff_percentage)) + '%'))
        else:
            temp_avg.append('N/A')
        #converts spin axis to tilt
        spin_axis = temp_avg[8]
        ...
        if tilt2<10:
            ...
            tilt2str = str(tilt2)
        temp_avg[8] = str(int(round(tilt1,0))) + ':' + tilt2str
        #appends average with temporary storage array
        #print(temp_avg)
